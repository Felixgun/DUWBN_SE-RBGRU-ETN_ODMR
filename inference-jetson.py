...
# !source ~/.bashrc
import cv2
print('path amount : ',len(sys.path))
...
# model = load_model(f'{model_folder}/gesture_model.h5')
...
dummy_input = np.random.random((1, 30, 4)).astype(np.float32)
...
def T2A(id,initiator):
    print('t2a: ',id)
    '''
    id = 0 : anchor num 1
    initiator : True or False
    '''
    i0 = ["DW17A0", {"x":4,"y":-3.15,"z":3,"quality":100},False,"17a0"]
    i1 = ["DW1E91", {"x":-4,"y":-3.15,"z":3,"quality":100},False,"1e91"]
    i2 = ["DW0476", {"x":0.45,"y":0.35,"z":1.5,"quality":100},False,"0476"]
    i3 = ["DW2FF3", {"x":-0.45,"y":-0.35,"z":1.5,"quality":100},False,"2ff3"]
    i4 = ["DW04CF", {"x":4,"y":3.15,"z":3,"quality":100},False,"04cf"]
    i5 = ["DW1E89", {"x":-4,"y":3.15,"z":3,"quality":100},False,"1e89"]
    i6 = ["DWD107", {"x":0.45,"y":6.65,"z":1.5,"quality":100},False,"d107"]
    i7 = ["DW0638", {"x":-0.45,"y":5.95,"z":1.5,"quality":100},False,"0638"]
    i8 = ["DW50A2", {"x":4,"y":9.45,"z":3,"quality":100},False,"50a2"]
    i9 = ["DW43A5", {"x":-4,"y":9.45,"z":3,"quality":100},False,"43a5"]
    ...
    if rule_flag == 0:
        if AreaNumTrue == 'a':
            ...
            if 0.7875*x + y > 6.3:       # 7->6   5->8
                ...
                p4,id4 = A2T(5)
                client = mqtt.Client()
                client.connect(RASP_IP, 1883, 60) # 設定連線資訊(IP, Port, 連線時間)
                client.publish("dwm/node/{}/downlink/config".format(id1), json.dumps(p1))
                client.publish("dwm/node/{}/downlink/config".format(id2), json.dumps(p2))
                client.publish("dwm/node/{}/downlink/config".format(id3), json.dumps(p3))
                client.publish("dwm/node/{}/downlink/config".format(id4), json.dumps(p4))
...
def reset_anchor():
    ...
    client = mqtt.Client()
    client.connect(RASP_IP, 1883, 60) 
    p1,id1 = T2A(reset_index[AreaNumTrue][0],False)
    client.publish("dwm/node/{}/downlink/config".format(id1), json.dumps(p1))
    p1,id1 = T2A(reset_index[AreaNumTrue][1],True)
    client.publish("dwm/node/{}/downlink/config".format(id1), json.dumps(p1))
    p1,id1 = T2A(reset_index[AreaNumTrue][2],False)
    client.publish("dwm/node/{}/downlink/config".format(id1), json.dumps(p1))
    p1,id1 = T2A(reset_index[AreaNumTrue][3],False)
    client.publish("dwm/node/{}/downlink/config".format(id1), json.dumps(p1))
...
input_index={'a':['0','2','4','5'],'b':['5','3','1','0'],'c':['4','6','8','9'],'d':['9','5','7','4']}

reset_index={'a':[0,2,4,5, 1,3,6,7,8,9],'b':[5,3,1,0, 2, 4, 6, 7, 8, 9],'c':[4,6,8,9, 0,1,2,3,5,7],'d':[9,5,7,4, 0,1,2,3,6,8]}
...
def predict_gesture(window_data):
    ...
    window_data = np.array(window).reshape(1, seq_length, 4).astype(np.float32)
...
def on_message(client, userdata, message):
    global data, data_m, numb_data, d, data_list,data_lock, new, d2, error_count
            ...
            if i + 1 < len(encoded_segments):
                ...
                if len(encoded_data) == 4 :
                    ...
                    if encoded_data[0] == encoded_data[1]:
                        ...
                        print('dup2')
                        encoded_data = encoded_data[:3]  # Remove the first of the duplicated characters
                        print(encoded_data)
                num = 0
                # Reverse the order of characters as per the encoding logic
                for char in reversed(encoded_data):
